...
- `narration` on a shot accepts **3000 characters**, the same limit the voice
  model reads; it took 4000 until 2026-09-15, and a line written to that limit
  stored fine, priced fine and failed at render. `voice_explainer` now also
  skips an over-long line at the door and names it in `skipped`, with the
  count. Split it across shots.
