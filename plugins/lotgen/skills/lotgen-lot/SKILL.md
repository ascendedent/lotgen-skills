...
- **Words on a still are data, not paint.** A new still is `text_mode:
  editable`: its prompt asks the model for a picture with no lettering, and
  the copy goes in `set_text_overlay` (`lines` for the default card, or your
  own `html`), which our worker renders over the clean picture for nothing.
  Change the words later and re-export; no regen. `get_project` shows each
  still's `text_mode` and `overlay` (absent, present or rendered). Keep a
  price or a headline OUT of the prompt on an editable still; on `baked` it
  stays in as before. `lettering_detected` on the still's asset means the
  model painted words anyway: regenerate before laying copy over them.
